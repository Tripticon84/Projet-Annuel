...
if (validateMandatoryParams($data, ['username', 'password','nom','prenom','type','tarif','date_debut_disponibilite','date_fin_disponibilite'])) {
    // Vérification si l'admin existe déjà
    $provider = getProviderByUsername($data['username']);
    if (!empty($provider)) {
        returnError(400, 'Provider already exist');
        return;
    }
