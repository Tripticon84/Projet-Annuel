...
try {
    ...
    // 3. Création d'un frais lié au devis
    logAction("Début de la création d'un frais pour le devis #$devis_id");
    $expenseData = [
        ...
        'est_abonnement' => 0, // Assurez-vous que c'est bien un frais ponctuel, pas un abonnement
    ...
    if ($curlError) {
        ...
        if ($result && isset($result['frais_id'])) {
            ...
            logAction("Création du frais réussie pour le devis #$devis_id. ID du frais: {$result['frais_id']}", "SUCCESS");
            
            // Toujours effectuer une liaison manuelle via l'API dédiée
            // même si le devis_id était déjà inclus dans la création du frais
            logAction("Liaison explicite entre le frais #{$frais_id} et le devis #{$devis_id}", "INFO");
            ...
            $curl = curl_init();
            curl_setopt($curl, CURLOPT_URL, "http://" . $_SERVER['HTTP_HOST'] . "/api/fees/link-devis.php");
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($curl, CURLOPT_CUSTOMREQUEST, "PUT");
            curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($linkData));
            curl_setopt($curl, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
            curl_setopt($curl, CURLOPT_SSL_VERIFYPEER, false);
            curl_setopt($curl, CURLOPT_SSL_VERIFYHOST, 0);
            ...
            if ($linkCurlError) {
                ...
                if ($linkHttpCode === 200 || $linkHttpCode === 201) {
                    logAction("Liaison du frais #{$frais_id} avec le devis #{$devis_id} réussie", "SUCCESS");
                    
                    // Vérifier les détails de la liaison
                    ...
                    // Extraire et logger le message d'erreur spécifique
                    ...
                    // Si l'erreur est due à une liaison déjà existante, ne pas la considérer comme une erreur
                    if (strpos($errorMsg, "déjà associé") !== false || 
                        strpos($errorMsg, "already linked") !== false) {
